
## Paid payout 0-872

- Status: paid / final on-chain reconciliation clean
- Range: 0-872
- Provider ID: 25
- Audit: runs/epoch-0-872-mrxmt825-56fc6d.json
- Final reconciliation: payout-state/reconcile/reconcile-0-872-final.json
- Done record: payout-state/paid-0-872.done.json
- Paid transfers: 18 / 18
- Delegator payout total: 136,500 AZTEC
- Operator retention according to audit: 45,500 AZTEC
- Transfer mode: direct ERC20.transfer from encrypted Foundry account
- Next payout should start from: 873

## Paid payout 873-1184

- Status: paid / final on-chain reconciliation clean
- Range: 873-1184
- Provider ID: 25
- Audit: runs/epoch-873-1184-ms9hzfn7-ff4916.json
- Final reconciliation: payout-state/reconcile/reconcile-873-1184-final.json
- Done record: payout-state/paid-873-1184.done.json
- Paid transfers: 19 / 19
- Delegator payout total: 141,750 AZTEC
- Operator retention according to audit: 47,250 AZTEC
- Transfer mode: direct ERC20.transfer from encrypted Foundry account
- Next payout should start from: 1185

## Paid payout 1185-1598

- Status: paid / final on-chain reconciliation clean
- Range: 1185-1598
- Provider ID: 25
- Audit: runs/epoch-1185-1598-mspbbvnb-68ff74.json
- Final reconciliation: payout-state/reconcile/reconcile-1185-1598-final.json
- Done record: payout-state/paid-1185-1598.done.json
- Paid transfers: 19 / 19
- Delegator payout total: 203,700 AZTEC
- Operator retention according to audit: 67,900 AZTEC
- Transfer mode: direct ERC20.transfer from encrypted Foundry account
- Next payout should start from: 1599

## Paid payout 1599-1895

- Status: paid / final on-chain reconciliation clean
- Range: 1599-1895
- Provider ID: 25
- Audit: runs/epoch-1599-1895-mt0l6d5g-fd7def.json
- Final reconciliation: payout-state/reconcile/reconcile-1599-1895-final.json
- Done record: payout-state/paid-1599-1895.done.json
- Paid transfers: 17 / 17
- Delegator payout total: 54,075 AZTEC
- Operator retention according to audit: 18,025 AZTEC
- Transfer mode: direct ERC20.transfer from encrypted Foundry account
- Next payout should start from: 1896

## Paid payout 1896-2558

- Status: paid / final on-chain reconciliation clean
- Range: 1896-2558
- Provider ID: 25
- Audit: runs/epoch-1896-2558-mtpx55dv-9a2657.json
- Final reconciliation: payout-state/reconcile/reconcile-1896-2558-final.json
- Done record: payout-state/paid-1896-2558.done.json
- Paid transfers: 16 / 16
- Delegator payout total: 11,550 AZTEC
- Operator retention according to audit: 142,450 AZTEC
- Transfer mode: direct ERC20.transfer from encrypted Foundry account
- Next payout should start from: 2559

## 2559-3015
- Delegator payout total: 8820 AZTEC
- Current-period operator retention: 108780 AZTEC
- Additional prior operator remainder settled: 2478.075024188583800884 AZTEC
- Transfer mode: direct ERC20.transfer
- Next payout should start from: 3016
